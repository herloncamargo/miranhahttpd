...
export function Privileged({ excuse, allowed, placement, tooltipId, children }) {
    ...
    if (!allowed) {
        contents = (
            <Tooltip
position={ placement || TooltipPosition.top} id={ tooltipId + "_tooltip" }
                     content={ excuse }
            >
                { contents }
            </Tooltip>);
...
export class PrivilegedButton extends React.Component {
    ...
    render() {
        return (
            <Privileged
allowed={ this.state.allowed } tooltipId={ this.props.tooltipId } placement={ this.props.placement }
                        excuse={ cockpit.format(this.props.excuse, this.props.permission.user ? this.props.permission.user.name : '') }
            >
                <Button
id={ this.props.buttonId } variant={ this.props.variant } onClick={ this.props.onClick }
